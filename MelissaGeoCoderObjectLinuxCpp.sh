...
# Name:    MelissaGeoCoderObjectLinuxCpp
# Purpose: Use the MelissaUpdater to make the MelissaGeoCoderObjectLinuxCpp code usable
...
CurrentPath=$(pwd)
ProjectPath="$CurrentPath/MelissaGeoCoderObjectLinuxCpp"
...
cd MelissaGeoCoderObjectLinuxCpp
make &> /dev/null
cd ..
...
if [ -z "$zip" ];
then 
    $BuildPath/MelissaGeoCoderObjectLinuxCpp --license $license --dataPath $DataPath
else
    $BuildPath/MelissaGeoCoderObjectLinuxCpp --license $license --dataPath $DataPath --zip "$zip"
fi
